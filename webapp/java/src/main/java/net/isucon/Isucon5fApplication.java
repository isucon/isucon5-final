    ...
    List<Map<String, Object>> data() {
        ...
        return arg.entrySet().stream().map(e -> {
            ...
            Endpoint row = db.queryForObject(
                    ...
                    (rs, i) -> {
                        Optional<String> tokenType = Optional.ofNullable(rs.getString("token_type"));
                        return new Endpoint(rs.getString("meth"), tokenType.map(Endpoint.TokenType::valueOf).orElse(null),
                                rs.getString("token_key"), rs.getString("uri"));
                    },
                    service);
            ...
            String uri = String.format(row.getUri(), List.class.cast(conf
                    .getOrDefault("keys", Collections.emptyList())).toArray());
                ...
                case "GET": {
                    return restTemplate.exchange(
                            UriComponentsBuilder.fromUriString(uri).queryParams(
                                    toMultiValueMap(params)).build().toUri(),
                            HttpMethod.GET, new HttpEntity(toMultiValueMap(headers)), new ParameterizedTypeReference<Map<String, Object>>() {
                            }).getBody();
                }
                case "POST": {
                    return restTemplate.exchange(URI.create(uri),
                            HttpMethod.POST, new HttpEntity(toMultiValueMap(
                                    params), toMultiValueMap(headers)), new ParameterizedTypeReference<Map<String, Object>>() {
                            }).getBody();
                }
                default:
